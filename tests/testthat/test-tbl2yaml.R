...
test_that("ok", {
  expect_silent(tbl2yaml(tbl, filename))
...
test_that("add cols", {
  expect_silent(tbl2yaml(tbl, filename, class = "PSYCH"))
  expect_true(file.exists(filename_yml))
  tbl2 <- read_marks(yaml = filename_yml) %>% as.data.frame()
...
test_that("overwrite cols", {
  expect_warning(tbl2yaml(tbl, filename, marker = "LISA"),
